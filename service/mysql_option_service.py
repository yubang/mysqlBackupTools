...
class MysqlOptionService:
    def get_db_list(self):
        ...
        s = os.popen(cmd).read()
        arrs = s.split("\n")
        return [obj for index, obj in enumerate(arrs) if index != 0 and obj and obj != 'information_schema' and obj != 'performance_schema']
    ...
    def backup_db(self, db):
        ...
        if config.backup_gzip:
            cmd = "mysqldump -u %(username)s -p%(password)s -P %(port)d -h %(host)s %(db)s | gzip > %(path)s.gz " % {
                "username": config.mysql_username,
                "password": config.mysql_password,
                "port": config.mysql_port,
                "host": config.mysql_host,
